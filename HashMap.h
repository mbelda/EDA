 ...
 *
 * Modificado por Isabel Pita 2016
 * Manejo de excepciones de la STL
 */
#ifndef __HASHMAP_H
#define __HASHMAP_H
 ...
 * Las operaciones son:
 *
 * - HashMapVacio: operaciÛn generadora que construye
 * una tabla hash vacÌa
 *
...
private:
	...
	public:
		/* Constructores. */
		Nodo(Clave const&clave, Valor const&valor) :
		_clave(clave), _valor(valor), _sig(nullptr) {};
        
		Nodo(Clave const&clave, Valor const&valor, Nodo *sig) :
...
public:
	...
	void insert(Clave const&clave, Valor const&valor) {
		// Si la ocupaciÛn es muy alta ampliamos la tabla
		float ocupacion = 100 * ((float) _numElems) / _tam;
		if (ocupacion > MAX_OCUPACION)
			amplia();
        
		// Obtenemos el Ìndice asociado a la clave.
		unsigned int ind = ::hash(clave) % _tam;
        
		// Si la clave ya existÌa, actualizamos su valor
		Nodo *nodo = buscaNodo(clave, _v[ind]);
		if (nodo != nullptr) {
			nodo->_valor = valor;
		} else {
			// Si la clave no existÌa, creamos un nuevo nodo y lo insertamos
			// al principio
	...
	/**
	 *	 OperaciÛn modificadora que elimina una clave de la tabla.
	 ...
	 */
	const Valor &at(Clave const&clave) const {
		// Obtenemos el Ìndice asociado a la clave.
		unsigned int ind = ::hash(clave) % _tam;
	 ...
	 *	 a esa clave.
	 */
	bool contains(Clave const&clave) const {
		// Obtenemos el Ìndice asociado a la clave.
		unsigned int ind = ::hash(clave) % _tam;
	...
	Valor &operator[](Clave const&clave) {
		...
		Nodo *nodo = buscaNodo(clave, _v[ind]);
		if (nodo == nullptr) {
			// No est·, lo aÒadimos y vemos dÛnde quedÛ.
			insert(clave, Valor());
	...
	public:
		void next() {
			...
			while ((_act == nullptr) && (_ind < _tabla->_tam - 1)) {
				++_ind;
				_act = _tabla->_v[_ind];
			}
		}
        
		Clave const&key() const {
			if (_act == nullptr) throw std::domain_error("acceso invalido");
			return _act->_clave;
		}
        
		Valor const&value() const {
			if (_act == nullptr) throw std::domain_error("acceso invalido");
			return _act->_valor;
		}
	...
	protected:
		...
		unsigned int _ind;      ///< Õndice actual en el vector _v
	};
	 ...
	 */
	ConstIterator find(Clave const&clave) const {
		// Obtenemos el Ìndice asociado a la clave.
		unsigned int ind = ::hash(clave) % _tam;
		...
		// si nodo == nullptr, devuelve en realidad cend()
	 ...
	 *	 clave, o al final del recorrido si clave no encontrada
	 */
	Iterator find(Clave const&clave) {
		// Obtenemos el Ìndice asociado a la clave.
		unsigned int ind = ::hash(clave) % _tam;
	...
	/** Constructor copia */
	HashMap(HashMap<Clave, Valor> const&other) {
		copia(other);
	}
    
	/** Operador de asignaciÛn */
	HashMap<Clave, Valor> &operator=(HashMap<Clave, Valor> const&other) {
		if (this != &other) {
			libera();
			copia(other);
		}
...
private:
	 ...
	 * a este mÈtodo se debe invocar al mÈtodo "libera".
	 *
	 * @param other tabla que se quiere copiar.
	 */
	void copia(HashMap<Clave,Valor> const&other) {
		_tam = other._tam;
	...
	/**
	 * Este mÈtodo duplica la capacidad del array de punteros actual.
	 */
	void amplia() {
		// Creamos un puntero al array actual y anotamos su tamaÒo.
		Nodo **vAnt = _v;
		unsigned int tamAnt = _tam;
        
		// Duplicamos el array en otra posiciÛn de memoria.
		_tam *= 2;
		_v = new Nodo*[_tam];
		for (unsigned int i=0; i<_tam; ++i)
			_v[i] = nullptr;
		...
		// posiciÛn que le corresponde en el nuevo array.
		for (unsigned int i=0; i<tamAnt; ++i) {
            
			// IMPORTANTE: Al modificar el tamaÒo tambiÈn se modifica el Ìndice
			...
			while (nodo != nullptr) {
				...
				// Calculamos el nuevo Ìndice del nodo, lo desenganchamos del
	 ...
	 * Si no lo encuentra "act" quedar· apuntando a nullptr.
	 *
	 * @param clave clave del nodo que se busca.
	 * @param act [in/out] inicialmente indica el primer nodo de la b˙squeda y
	 *            al finalizar indica el nodo encontrado o nullptr.
	 * @param ant [out] puntero al nodo anterior a "act" o nullptr.
	 */
	static void buscaNodo(Clave const&clave, Nodo* &act, Nodo* &ant) {
		ant = nullptr;
	 ...
	 * diferencia del otro mÈtodo "buscaNodo", este no devuelve un puntero al
	 * nodo anterior.
	 *
	 * @param clave clave del nodo que se busca.
	 ...
	 */
	static Nodo* buscaNodo(Clave const&clave, Nodo* prim) {
		Nodo *act = prim;
		Nodo *ant = nullptr;
		buscaNodo(clave, act, ant);
		return act;
	...
	/**
	 * OcupaciÛn m·xima permitida antes de ampliar la tabla en tanto por cientos.
	 */
	static const unsigned int MAX_OCUPACION = 80;
    
    
	Nodo **_v;               ///< Array de punteros a Nodo.
	unsigned int _tam;       ///< TamaÒo del array _v.
